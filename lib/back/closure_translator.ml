...
let ff_make_bool = C.VARIABLE "ff_make_int"
...
let make_c_ident x = to_c_ident Ident.(to_string (create ~hint:x))
...
and trans_expr ctx e =
  ...
  | ELet (x, e0, e1) ->
      ...
      let x, ctx = add_var ~need_decl:true x ctx in
      let result_stmt =
      ...
      (e1_v, result_stmt)
  | EConst (S.CInt i) ->
      let ret_v = add_decl "temp" ctx in
      ( ret_v,
        [
  ...
  | ETuple es ->
      ...
      (tu, stmts)
  | EIf (e0, e1, e2) ->
      let ifel_v = add_decl "ifel_res" ctx in
      let e0_v, e0_stmts = trans_expr ctx e0 in
      let e1_v, e1_stmts = trans_expr ctx e1 in
  ...
  | EApp (e0, e1) ->
      let app_v = add_decl "app_res" ctx in
      let e0_v, e0_stmts = trans_expr ctx e0 in
      let e1_v, e1_stmts = trans_expr ctx e1 in
      ( app_v,
        e0_stmts @ e1_stmts
  ...
  | ECons i ->
      let constr_v = add_decl (Printf.sprintf "constr%d" i) ctx in
      ( constr_v,
        [
  ...
  | EField (e, name) ->
      let field = add_decl "field" ctx in
  ...
  | EClosure (fvs, cfunc) ->
      let clos_v = add_decl "clos" ctx in
      let fvs_c = List.map (fun fv -> List.assoc fv ctx.dict) fvs in
      let cfn_name = make_c_ident (Ident.to_string cfunc) in
      ( clos_v,
        [
        ...
        List.fold_left
          (fun (stmts_acc, mems_c_acc, ctx) mem ->
            ...
            | FSimple (x, e) ->
                ...
                let x_c, ctx = add_var ~need_decl:true x ctx in
                ( stmts_acc @ e_stmts
  ...
  let binds_c, ctx =
    List.fold_left
      (fun (acc_binds_c, ctx) (x, _) ->
        let bind_c, ctx = add_var ~need_decl:true x ctx in
           ...
           ( ff_fill_letrec_closure,
             [
               ...
               make_compound ff_obj_typename
                 (List.map
                    (fun (_, cfunc) ->
                      C.VARIABLE (to_c_ident (Ident.to_string cfunc)))
                    binds);
...
and trans_fn (cfunc, fvs, para, e) =
  ...
  let cfn_name = make_c_ident (Ident.to_string cfunc) in
  let c_para, ctx = add_var ~need_decl:false para ctx in
  let fvs_para, ctx = add_var ~need_decl:false "fvs" ctx in
...
let create_main e =
  ...
  let cfn_name = make_c_ident "main" in
  let var, body = trans_expr ctx e in
  let var_decls = get_var_decls ctx in
  let cfn_body = List.append body [ C.RETURN (C.VARIABLE var) ] in
  ( cfn_name,
    C.FUNDEF
      ( (cfn_name, C.PROTO (ff_obj_typename, [])),
        ( var_decls,
          List_utils.fold_left_first
            (fun s0 s1 -> C.SEQUENCE (s0, s1))
            cfn_body ) ) )
...
let translate (e, (fns : func list)) =
  let main_name, main_def = create_main e in
  let fns = main_def :: List.map trans_fn fns in
  let buf = Buffer.create 50 in
  Cprint1.print buf fns;
